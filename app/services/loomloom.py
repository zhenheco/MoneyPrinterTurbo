...
class LoomLoomSettings:
    ...
    def from_mapping(
        ...
        raw_allowed_hosts = values.get("loomloom_artifact_allowed_hosts", ())
        ...
        elif isinstance(raw_allowed_hosts, (list, tuple, set)):
            allowed_hosts = tuple(
        ...
        settings = cls(
            ...
            artifact_allowed_hosts=allowed_hosts,
        )
    ...
    def validate(self, *, require_api_token: bool = True) -> None:
        ...
        if parsed.scheme != "https" or not parsed.netloc:
            raise LoomLoomConfigurationError(
                "loomloom_base_url must be an absolute HTTPS URL"
            )
        if parsed.username or parsed.password:
            raise LoomLoomConfigurationError(
                "loomloom_base_url must not contain embedded credentials"
            )
        for host in self.artifact_allowed_hosts:
            if not host or any(character in host for character in "/?#@"):
                raise LoomLoomConfigurationError(
                    "loomloom_artifact_allowed_hosts contains an invalid host"
                )
        for name, value in (
...
class LoomLoomConfirmedVideoRequest:
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.quote, LoomLoomQuote):
            raise LoomLoomConfigurationError("a current LoomLoom quote is required")
        if self.quote.listing_version_id != str(self.listing_version_id).strip():
            raise LoomLoomConfigurationError("quote listing version does not match request")
        if self.quote.input_rows != self.batch.input_rows:
            raise LoomLoomConfigurationError("quote input rows do not match request")
        if self.quote.task_count != len(self.batch.input_rows):
            raise LoomLoomConfigurationError("quote task count does not match request")
        if int(self.budget_limit_t or 0) <= 0:
            raise LoomLoomConfigurationError(
                "a positive LoomLoom budget limit in T is required"
            )
        if self.quote.estimated_buyer_payable_t > int(self.budget_limit_t):
            raise LoomLoomConfigurationError(
                "LoomLoom quote exceeds the configured budget limit"
            )
...
class LoomLoomVideoBackend(LoomLoomScriptBackend):
    ...
    def _video_artifact(self, row: Mapping[str, Any]) -> dict[str, Any]:
        ...
        self._validate_artifact_url(access_url)
    ...
    def _download_video_artifact(self, access_url: str, destination: str) -> None:
        ...
        downloaded_bytes = 0
        response = None
        current_url = self._validate_artifact_url(access_url)
        try:
            for redirect_count in range(MAX_ARTIFACT_REDIRECTS + 1):
                ...
                if 300 <= response.status_code < 400:
                    ...
                    if not location:
                        raise LoomLoomAPIError(
                            "video artifact redirect is missing Location"
                        )
                    if redirect_count >= MAX_ARTIFACT_REDIRECTS:
                        raise LoomLoomAPIError(
                            "video artifact exceeded the redirect limit"
                        )
                    current_url = self._validate_artifact_url(
                        urljoin(current_url, location)
                    )
                    continue
                break
            else:  # pragma: no cover - loop exits via break or raises
                raise LoomLoomAPIError("video artifact download did not resolve")
    ...
    def _validate_artifact_url(self, access_url: str) -> str:
        parsed = urlsplit(str(access_url or "").strip())
        if parsed.scheme != "https" or not parsed.netloc:
            raise LoomLoomAPIError("video artifact accessUrl must be an HTTPS URL")
        if parsed.username or parsed.password:
            raise LoomLoomAPIError("video artifact accessUrl must not contain credentials")
        hostname = (parsed.hostname or "").lower().rstrip(".")
        if not hostname:
            raise LoomLoomAPIError("video artifact accessUrl must contain a hostname")

        configured_hosts = self.settings.artifact_allowed_hosts
        if configured_hosts:
            allowed = any(
                hostname == host or hostname.endswith(f".{host.lstrip('*.')}")
                for host in configured_hosts
            )
        else:
            base_hostname = (urlsplit(self.settings.base_url).hostname or "").lower()
            allowed = hostname == base_hostname.rstrip(".")
        if not allowed:
            raise LoomLoomAPIError(
                f"video artifact host is not allowlisted: {hostname}"
            )

        if hostname in {"localhost", "localhost.localdomain"} or hostname.endswith(
            ".local"
        ):
            raise LoomLoomAPIError("video artifact host must not be local")
        try:
        ...
        if any(
            ...
            raise LoomLoomAPIError("video artifact host resolves to a private address")
        return str(access_url).strip()
