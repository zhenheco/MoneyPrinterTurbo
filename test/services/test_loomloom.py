...
class TestLoomLoomSettings(unittest.TestCase):
    ...
    def test_rejects_http_base_url_before_sending_bearer_credentials(self):
        with self.assertRaisesRegex(LoomLoomConfigurationError, "HTTPS"):
            LoomLoomSettings.from_mapping(
                {
                    "loomloom_base_url": "http://example.test/loom/v1",
                    "loomloom_api_token": "secret-token",
                }
            )
...
class TestLoomLoomConfirmedRequest(unittest.TestCase):
    def test_rejects_quote_over_budget_before_execute(self):
        batch = LoomLoomVideoBatch(input_rows=(
            {"scenePrompt": "coffee", "aspectRatio": "9:16"},
        ))
        request = LoomLoomConfirmedVideoRequest(
            ...
            quote=LoomLoomQuote(
                ...
                currency="T",
                task_count=1,
                estimated_buyer_payable_t=3,
                estimated_buyer_payable_amount="0.003",
                input_rows=batch.input_rows,
            ),
            budget_limit_t=2,
        )

        with self.assertRaisesRegex(LoomLoomConfigurationError, "exceeds"):
            request.validate()
...
class TestLoomLoomVideoBackend(unittest.TestCase):
    def setUp(self):
        self.settings = LoomLoomSettings(
            ...
            artifact_allowed_hosts=("objects.test",),
        )
        self.session = MagicMock()
    ...
    def test_rejects_non_https_artifact_url(self):
        self.session.request.return_value = _Response(
            200,
            {
                "items": [
                        ...
                        "artifacts": [
                            {
                                "portName": "output",
                                "mimeType": "video/mp4",
                                "accessUrl": "http://objects.test/video.mp4",
                            }
                        ],
                    }
                ]
        ...
        with tempfile.TemporaryDirectory() as directory:
            with self.assertRaisesRegex(LoomLoomAPIError, "HTTPS"):
                self.backend.download_video_results("run-1", directory)

    def test_rejects_artifact_host_outside_allowlist(self):
        self.session.request.return_value = _Response(
            200,
            {
                "items": [
                        ...
                        "artifacts": [
                            {
                                "portName": "output",
                                "mimeType": "video/mp4",
                                "accessUrl": "https://evil.test/video.mp4",
                            }
                        ],
                    }
                ]
        ...
        with tempfile.TemporaryDirectory() as directory:
            with self.assertRaisesRegex(LoomLoomAPIError, "not allowlisted"):
                self.backend.download_video_results("run-1", directory)
