...
3. Qwen OAuth free tier 已於 2026-04-15 停止；Qwen Code session token 不列入新流程依賴，需改用 Coding Plan／其他 provider。若本機仍有舊快取，只能作為待驗證的人工狀態，不得寫入自動化設定。官方 ModelStudio Token Plan endpoint 的 Qwen Code CLI Assisted 路徑已驗證可用；這不等同 MoneyPrinterTurbo backend Automated-ready。`modelstudio_token_plan` 仍是 Automated candidate，並維持 `manual_action_required`，須待 ProviderEvent／UsageLedger／cost／request-id contract 實作並通過 smoke 測試後，才可提升狀態。
4. 每次選擇都必須先通過 `auth_mode`、`execution_mode`、模型能力、成本與權限檢查；不可因某個 CLI 已安裝就宣稱 Provider 可供背景 Job 使用。

截至 2026-08-16 的本機 capability probe：Gemini CLI 0.47.0 的 OAuth code flow 曾到達授權頁並將 code 送回 CLI，但 CLI 明確回報「This client is no longer supported for Gemini Code Assist for individuals」；因此 Gemini 是 `provider_unavailable`（原因：`client_deprecated`），不是 `assisted_ready`，且 Antigravity 不列入本次已驗證能力。Grok Build 1.0.0 已登入，`grok models` 可列出模型且 Assisted 最小 smoke 回傳 `OK`。Qwen Code 0.21.2 透過 checked-in launcher `autoflow-qwen-token-plan.sh`，使用 model `qwen3.8-max` 與 Alibaba ModelStudio Token Plan endpoint `https://token-plan.ap-southeast-1.maas.aliyuncs.com/compatible-mode/v1`，對 `AUTOFLOW_QWEN_ACCOUNT=account-1` 與 `AUTOFLOW_QWEN_ACCOUNT=account-2` 各完成 1 次明確的 non-interactive request，均回傳 `OK`，每次 1 API request、0 errors；launcher 隔離 `QWEN_HOME`，並在執行時注入明確選定的帳號認證資料，無 silent fallback。這證明 Qwen Code／Token Plan CLI 的 Assisted 路徑對兩個帳號可用，但不證明 MoneyPrinterTurbo backend 的 Automated readiness；目前尚未執行 ProviderEvent／UsageLedger／cost／request-id contract smoke，因此 `modelstudio_token_plan` 仍為 Automated candidate、狀態為 `manual_action_required`，直到該 contract 實作並測試完成。Qwen Code 0.21.2 的另一個明確 non-interactive smoke 使用 `--auth-type qwen-oauth` 回報 `Qwen OAuth free tier discontinued on 2026-04-15`，需改用 Coding Plan／其他 provider。詳細證據見 `docs/reports/provider-capability-2026-08-16.md`。
